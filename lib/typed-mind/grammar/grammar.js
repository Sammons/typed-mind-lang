...
module.exports = grammar({
  ...
  rules: {
    ...
    typedef_declaration: ($) =>
      seq($.entity_name, '=', choice($.typedef_enum_variant, $.type_expr), optional($.inline_comment), $._line_end),
    typedef_declaration_final: ($) => seq($.entity_name, '=', choice($.typedef_enum_variant, $.type_expr), optional($.inline_comment)),
    ...
    _paramlist_opaque_run: ($) => prec.right(seq($._paramlist_opaque_open, repeat($._opaque_piece), ')', repeat($._opaque_piece))),
    ...
    // begins after the type has closed. So the string alternative lives on
    // `_opaque_group_piece` (reachable only once a group is open) and is
    // absent from `_opaque_piece` (the top-level run). The two rules are
    // otherwise identical and must stay in lockstep — the ONLY intended
    // difference is the string alternative.
    //
    ...
    // Corpus proof: all 264 `.tmd` files in the repo produce byte-identical
    // S-expressions before and after, including the 38 that already carried
    // pre-existing ERROR nodes from unrelated knownGaps.
    _opaque_piece: ($) =>
      choice(
        $._opaque_paren_group,
    ...
    // in what they accept.
    //
    // The PAREN group deliberately keeps the bare `_opaque_piece` body. The
    // `(` position is the one place where a second production is already
    // live — `_paramlist_opaque_run` (issue #50's `(x: number) => void`
    // fix) — and both loops recurse through the identical `_opaque_piece`
    // symbol. That identity is exactly what the `prec.right` comment above
    // relies on to stay unambiguous; giving the paren group a DIFFERENT
    // repeat body breaks the identity and produces a real reduce-reduce
    // conflict between `_opaque_paren_group_repeat1` and
    // `_paramlist_opaque_run_repeat1` (confirmed: `tree-sitter generate`
    // fails with precisely that pair). Braces and brackets have no
    // competing production at their opening token, so they take the string
    // alternative safely.
    //
    // Residual, deliberately left open and empirically bounded: a quoted
    // literal inside a paren group that is itself nested in an opaque run —
    // `{ cb: (x: "a") => void; b: string }` — still ERRORs, because that
    // inner `(` is consumed by `_opaque_paren_group`, which keeps the bare
    // body. Note this is NARROWER than "parens reject strings": a top-level
    // `("a" | "b")` parses cleanly, since it routes through the structured
    // `_type_paren_group`, not the opaque path (verified both ways). No
    // corpus instance of the nested form exists (grep-verified) and every
    // shape fixture 93 targets is brace-delimited, so closing it is not
    // worth reopening the issue-50 ambiguity. Both the working top-level
    // form and the residual nested form are pinned as corpus tests so the
    // boundary is a committed fact rather than an accident.
    // No `conflicts` block: the grammar has none and gains none.
