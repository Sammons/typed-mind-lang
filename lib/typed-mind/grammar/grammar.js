...
// readonly-array keyword. Two concrete compound tokens cover the two legal
// continuations after `readonly `'s whitespace (an identifier's first
// character, or the `(` that opens a parenthesized group) because — per the
// H10 precedent (header_quoted_name) — the token can only bake in ONE
// disambiguating suffix character, and that character's grammar-visible
// re-lexing differs by shape (identifier rest vs. a flat paren-group rest,
// exactly mirroring `_paren_group`'s non-recursive one-level grouping).
// Adversarial case, answered (doc §1): `readonly[]` has no whitespace-plus-
// [identifier-start|`(`] after the keyword, so NEITHER compound token
// matches, and the text lexes as `type_named` "readonly" + an array suffix —
// the two readings never collide.
