...
module.exports = grammar({
  ...
  rules: {
    ...
    // `true`/`false` are also valid `entity_name`s and match the identical
    // span as `bool_literal` for those two words — an equal-length lexer
    // tie that `block_property`'s choice ORDER cannot settle on its own
    // (order only arbitrates between grammar rules once the lexer has
    // already returned a token). `bool_literal`'s `prec(1)` is what
    // actually wins P4 the tie; see its definition.
    property_identifier: ($) => seq($._block_indent, $.property_key, ':', $.entity_name, optional($.inline_comment), $._newline),
    ...
    inline_field_pair: ($) => seq($.property_key, ':', choice($.string, $.bool_literal, $.entity_name)),
    ...
    // prec(1): `true`/`false` are equal-length matches against `entity_name`
    // (both 4/5-char identifiers), so the lexer's plain longest-match tie
    // break does not reliably favor bool_literal — observed picking
    // property_identifier for `required: true` despite property_bool being
    // listed earlier in `block_property`'s choice (choice ORDER only
    // arbitrates between grammar rules once the lexer has already handed
    // back a token; it cannot promote a tied-length token over another).
    // Raising bool_literal's precedence settles the tie explicitly.
