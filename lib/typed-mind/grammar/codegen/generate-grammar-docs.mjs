...
const buildMarkdown = (grammarJson) => {
  ...
  sections.push('');
  sections.push(
    'Each continuation attaches only to the entity kinds that carry its property; the checker ' +
      'rejects the others (`semantics/illegal-continuation`). `~> [...]` attaches to Function, Class, ' +
      'ClassFile and Constants; `$< [...]` attaches to Function, Class and ClassFile. On a Class or ' +
      'ClassFile, a calls or consumes entry means a member body of that class calls or reads the target ' +
      '(the slot is per class, not per member).',
  );
  sections.push('');
  sections.push(
    'An import entry is `Owner.name` when more than one File exports or re-exports `name`; otherwise it is bare. ' +
      'Hand-authors may always qualify. `Owner` is the File the name was imported from, so the checker credits a ' +
      're-exporting File only through an entry that names it (`DateUtilsFile.formatDate` below).',
  );
  sections.push('');
  ...
  sections.push('');
  sections.push(
    'Class and ClassFile blocks carry typed members as quoted properties: `method: "name(params) => Return"`, ' +
      '`constructor: "(params)"` and `property: "[readonly] name[?]: Type"` (RFC-TM-14 R3a; the property payload is ' +
      'the DTO field shape). The checker resolves the type names inside them. Shortform has no member slot, so an ' +
      'entity with typed members always emits in longform.',
  );
  sections.push('');
