...
//
// RFC-TM-4 §3 (rfc-tm-4-diamond.md): also wired as this package's `prebuild`
// (package.json), not only `pretest`. Before the flip, `pnpm build` alone
// (with no `pnpm test` run first) left grammar.wasm absent, which was fine —
// the legacy DSLChecker bridge never touches wasm. TypedMind.create() does:
// the CLI's --check path and the static website's snippet-validation prebuild
// chain both now call it at runtime after a plain `build`, so the wasm must
// exist by the end of `pnpm build`, not only `pnpm test`.
