...
// ../../node_modules/.pnpm/tsup@8.5.0_postcss@8.5.6_supports-color@10.2.2_tsx@4.20.5_typescript@6.0.3/node_modules/tsup/assets/cjs_shims.js
...
var QualifiedNameResolver = class {
  ...
  resolve(name2, options = {}) {
    ...
    while (separator > 0 && !this.#byName.has(name2.slice(0, separator))) {
    ...
    if (owner instanceof FileNode || owner instanceof ClassFileNode) {
      ...
      const reExported = owner instanceof FileNode && (owner.reExports.includes(name2) || owner.reExports.includes(member));
      if (declared !== void 0) {
    ...
    if (owner instanceof ConstantsNode) {
      const schema = owner.schema === void 0 ? void 0 : this.#byName.get(owner.schema);
      return schema instanceof DtoNode && this.resolve(schema.name).kind === "entity" && schema.fields.some((field) => field.name === member) ? { kind: "member", owner, member } : failure("missing-member");
    }
...
var quoteSwapOccurred = (text) => text.includes('"');
...
// ../typed-mind/dist/emitter/emitter-diagnostics.js
var QUOTE_SWAP_CODE = "emitter/quote-swap";
var UNREPRESENTABLE_ALIAS_CODE = "emitter/unrepresentable-alias";
var unrepresentableAliasDiagnostic = (entityName, printedType, span) => {
  return {
...
var opensWithStringLiteral = (printed) => printed.trimStart().startsWith('"');
var quoteSwapDiagnostic = (entityName, propertyName, span) => {
  return {
...
var pushIfSwapped = (diagnostics, entityName, propertyName, value, span) => {
  if (value !== void 0 && quoteSwapOccurred(value)) {
    diagnostics.push(quoteSwapDiagnostic(entityName, propertyName, span));
  }
};
var swappedStringLiteralsIn = (node) => {
  switch (node.kind) {
    case "literal":
...
var pushIfTypeExprSwapped = (diagnostics, entityName, propertyName, typeExpr, span) => {
  if (typeExpr === void 0) {
    return;
  }
...
var quoteSwapDiagnosticsFor = (entity, form) => {
  const diagnostics = [];
  if (form === "longform") {
  ...
  switch (entity.kind) {
    case "Program":
      pushIfSwapped(diagnostics, entity.name, "purpose", entity.purpose, entity.span);
      break;
    case "File":
      pushIfSwapped(diagnostics, entity.name, "purpose", entity.purpose, entity.span);
      break;
    case "Function":
      pushIfSwapped(diagnostics, entity.name, "description", entity.description, entity.span);
      break;
    case "Class":
      pushIfSwapped(diagnostics, entity.name, "purpose", entity.purpose, entity.span);
      break;
    ...
    case "TypeDef": {
      ...
      if (typeDef.variant === "alias") {
        ...
        if (form === "longform" && typeDef.aliasType !== void 0) {
...
var functionToLongform = (entity) => {
  ...
  if (entity.description !== void 0) {
    body2.push(`description: ${quoteStringLiteral(entity.description)}`);
  }
...
var dtoFieldToLongform = (field) => {
  ...
  if (field.description !== void 0) {
    body2.push(`description: ${quoteStringLiteral(field.description)}`);
  }
...
var assetToLongform = (entity) => {
  const body2 = [`type: Asset`, `description: ${quoteStringLiteral(entity.description)}`];
...
var uiComponentToLongform = (entity) => {
  const body2 = [`type: UIComponent`, `description: ${quoteStringLiteral(entity.purpose)}`];
  if (entity.root) {
...
var runParameterToLongform = (entity) => {
  const body2 = [`type: ${entity.paramType}`, `description: ${quoteStringLiteral(entity.description)}`];
  if (entity.defaultValue !== void 0) {
    body2.push(`default: ${quoteStringLiteral(entity.defaultValue)}`);
  }
...
var typeDefToShortform = (entity) => {
  ...
  return [`${entity.name} = ${entity.aliasType === void 0 ? "" : printTypeExpr(entity.aliasType)}`];
};
...
var quoteSwapDiagnosticsForSuppressions = (suppressions) => {
  const diagnostics = [];
...
var checkInheritanceChains = (context) => {
  ...
  for (const [name2, entity] of context.byName) {
    ...
    if (entity.extends !== void 0) {
      ...
      } else if (context.names.target(entity.extends) === void 0) {
        context.addFinding({
    ...
    for (const implemented of entity.implements) {
      if (context.names.target(implemented) === void 0) {
        context.addFinding({
...
var collectTypeExprReferences = (node, referenced, names) => {
  switch (node.kind) {
    case "named":
...
var openProgram = (syntaxNode) => {
  ...
  if (purposeText !== void 0) {
    accumulator.slots.purpose = unquote2(purposeText);
  }
...
var openAsset = (syntaxNode) => {
  ...
  accumulator.slots.description = unquote2(declaration.stringChildren().at(0)?.text ?? '""');
  return accumulator;
};
var openUiComponent = (syntaxNode) => {
  ...
  accumulator.slots.purpose = unquote2(declaration.stringChildren().at(0)?.text ?? '""');
  let root = false;
  for (let childIndex = 0; childIndex < syntaxNode.childCount; childIndex++) {
    const child = syntaxNode.child(childIndex);
    if (child !== null && !child.isNamed && child.type === "&!") {
  ...
  if (inline !== void 0) {
    ...
    properties = inline.inlineFieldPairChildren().map((pair) => {
      ...
      if (stringValue !== void 0) {
        return { kind: "scalar", key, value: unquote3(stringValue), span: pairSpan };
      }
...
var classifyBlockProperty = (property) => {
  ...
  if (stringProperty !== void 0) {
    return {
      ...
      value: unquote3(stringProperty.stringChildren().at(0)?.text ?? '""'),
      span
    };
  }
...
var NameOccurrenceIndex = class {
  ...
  constructor(cst, entities = []) {
    ...
    this.#occurrences = occurrences.map((occurrence) => {
      ...
      return owner === void 0 ? occurrence : { ...occurrence, exportingOwner: owner.name };
    });
    const byName = /* @__PURE__ */ new Map();
...
var targetOfOccurrence = (occurrence, names) => {
  return occurrence.exportingOwner === void 0 ? names.target(occurrence.name) : resolvedNameTarget(names.resolveExport(occurrence.exportingOwner, occurrence.name));
};
...
var provideReferencesForName = (uri, name2, nameIndex, names, exportingOwner) => {
  const target = names === void 0 ? void 0 : exportingOwner === void 0 ? names.target(name2)?.name : resolvedNameTarget(names.resolveExport(exportingOwner, name2))?.name;
