  ...
  resolveImports(imports, basePath) {
    ...
    for (const importStatement of imports) {
      ...
      if (document.imports.length > 0) {
        const nested = this.resolveImports(document.imports, (0, import_node_path.dirname)(fullPath));
...
var resolveDefaultWasmPath = async () => {
  ...
  const thisDir = dirname4(fileURLToPath2(import_meta2.url));
  const candidates = [
    join2(thisDir, "..", "..", "grammar", "grammar.wasm"),
    join2(thisDir, "..", "..", "grammar.wasm")
  ];
...
var resolveCorePackageRuntimeWasm = () => {
  ...
  try {
    webTreeSitterDir = (0, import_node_path3.dirname)(require.resolve("web-tree-sitter", { paths: [typedMindPackageDir] }));
  } catch {
    return void 0;
  }
...
var resolveWasmPaths = () => {
  const bundleAdjacentGrammarWasm = (0, import_node_path3.join)(__dirname, "grammar.wasm");
  const bundleAdjacentRuntimeWasm = (0, import_node_path3.join)(__dirname, "web-tree-sitter.wasm");
