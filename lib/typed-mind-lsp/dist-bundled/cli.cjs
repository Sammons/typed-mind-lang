...
var parseSignatureText = (text, options = {}) => {
  ...
  if (text[state.index] === "<") {
    ...
    const stops2 = [...binders.delimiters.filter((delimiter) => delimiter.char === ",").map((delimiter) => delimiter.index), binders.end];
    const cursor2 = { index: state.index + 1 };
    for (const stop2 of stops2) {
      const binder = text.slice(cursor2.index, stop2).trim();
      if (binder === "" && stop2 === binders.end && typeParameterNames.length > 0) {
        break;
      }
...
// ../typed-mind/dist/checker/collect-signature-references.js
init_cjs_shims();
var collectType = (type, referenced, binders) => {
  switch (type.kind) {
    case "named":
...
var collectPosition = (position, referenced, binders) => {
...
var collectSignatureReferences = (signature, referenced, outerBinders = /* @__PURE__ */ new Set()) => {
  const binders = /* @__PURE__ */ new Set([...outerBinders, ...signature.typeParameterNames]);
