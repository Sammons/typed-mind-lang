...
// RFC-TM-7 §2 (rfc-tm-7-diamond.md, S-CONS-WEB-1) — the playground's browser
// build: tsc-emitted ESM (tsconfig.browser.json) run here so the site build
// always ships a fresh dist-browser/, then copied into dist/assets/ alongside
// the same-origin vendored web-tree-sitter ESM runtime (the .js file
// specifically, never the package dir — it also ships a .cjs build) and its
// .wasm. grammar.wasm itself is copied by the deploy workflow's own step
// (deploy-static-site.yml) after this script runs; `pnpm serve` picks it up
...
const distBrowserSrc = path.join(typedMindPkgDir, 'dist-browser');
const distBrowserDest = path.join(distAssetsDir, 'dist-browser');
if (!fs.existsSync(distBrowserSrc)) {
  console.log(`${colors.red}Error: ${distBrowserSrc} not found after browser build${colors.reset}`);
  process.exit(1);
}
copyDirectory(distBrowserSrc, distBrowserDest);
console.log('Copied dist-browser/ to dist/assets/dist-browser/');

// Vendor web-tree-sitter's ESM runtime (the exact file the package's `import`
// export condition names) + its .wasm. Resolved relative to lib/typed-mind's
...
if (!fs.existsSync(webTreeSitterJsSrc)) {
  console.log(`${colors.red}Error: expected web-tree-sitter ESM file not found at ${webTreeSitterJsSrc}${colors.reset}`);
  process.exit(1);
}
fs.copyFileSync(webTreeSitterJsSrc, path.join(distAssetsDir, 'web-tree-sitter.js'));
fs.copyFileSync(webTreeSitterWasmSrc, path.join(distAssetsDir, 'web-tree-sitter.wasm'));
console.log('Copied web-tree-sitter.js + web-tree-sitter.wasm to dist/assets/');

// The grammar (language) wasm — distinct from web-tree-sitter's own runtime
// wasm above. Built by lib/typed-mind's own prebuild (scripts/build-wasm.mjs)
// via its normal `pnpm build`/`pnpm test` lifecycle, which this website
// package's own `prebuild` already triggers (`cd ../typed-mind && pnpm build`).
// The deploy workflow rebuilds grammar.wasm from the frozen grammar/ dir
// separately (deploy-static-site.yml, RFC-TM-1 L6) and copies it too, so a
// missing local copy here does not fail the deploy — it fails a local
// `pnpm serve` in a way that names exactly what to run first.
const grammarWasmSrc = path.join(typedMindPkgDir, 'grammar', 'grammar.wasm');
if (fs.existsSync(grammarWasmSrc)) {
  fs.copyFileSync(grammarWasmSrc, path.join(distAssetsDir, 'grammar.wasm'));
  console.log('Copied grammar.wasm to dist/assets/');
} else {
  console.log(
    `${colors.yellow}Warning: ${grammarWasmSrc} not found — run \`pnpm --dir ../typed-mind build\` first for a local playground to load grammar.wasm${colors.reset}`,
  );
}
...
console.log(`\n${colors.bright}Step 4: Vendoring third-party CDN assets${colors.reset}`);
...
try {
  ...
  console.log(`${colors.red}Error generating diagnostics page: ${err.message}${colors.reset}`);
  process.exit(1);
}
