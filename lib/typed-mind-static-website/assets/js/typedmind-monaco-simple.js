...
function validateCode(editor) {
  ...
  lastValidationTimeout = setTimeout(() => {
    ...
    updateProblemsPanel((result.diagnostics || []).map(diagnostic => ({
      message: diagnostic.message,
      ...
      try {
        ...
        document.querySelectorAll('.syntax-toggle .toggle-btn').forEach(function(btn) {
          btn.addEventListener('click', function() {
            ...
            // correctly. Falls back to the static examples only on a genuine
            // failure (parser not ready yet, empty content, a parse error the
            // emitter can't recover from — e.g. the known grammar-precedence
            // gap tracked as typed-mind-lang#103).
            if (window.typedMindEditor && window.typedMindBrowser) {
              try {
                ...
                if (convertedCode.trim()) {
                  ...
                  // Fallback to examples if conversion produces empty content
                  // (e.g. an empty editor).
                ...
                // Fallback to examples if conversion fails (parse error).
                if (TYPEDMIND_EXAMPLES[targetSyntax]) {
                  window.typedMindEditor.setValue(TYPEDMIND_EXAMPLES[targetSyntax]);
                }
              }
            } else {
              // Fallback to examples if the browser runtime isn't ready yet.
              if (TYPEDMIND_EXAMPLES[targetSyntax]) {
                window.typedMindEditor.setValue(TYPEDMIND_EXAMPLES[targetSyntax]);
              }
            }
