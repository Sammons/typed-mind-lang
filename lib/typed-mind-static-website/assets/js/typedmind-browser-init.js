...
// emitShortform(source), emitLongform(source), toggleFormat(source),
// detectFormat(source) } for typedmind-monaco-simple.js to consume, and
// dispatches a `typedmind-ready` event once construction finishes. All paths
// are same-origin (no CDN, no bundler) per the doc's Q5 offline answer.
//
// toggle-fidelity audit (2026-08-31, claude-home knowledge/projects/typedmind/
// toggle-fidelity-audit-2026-08-31.md) — emitShortform/emitLongform/
// toggleFormat/detectFormat added: TypedMindBrowser only exposed check/parse
// before, so the playground's toggle buttons fell back to a hand-rolled,
// drifted reimplementation (deleted, see typedmind-monaco-simple.js) instead
// of the real SyntaxEmitter this facade now exposes.
...
(async () => {
  ...
  window.typedMindBrowser = {
    ...
    emitShortform: (source) => instance.emitShortform(source),
    emitLongform: (source) => instance.emitLongform(source),
    toggleFormat: (source) => instance.toggleFormat(source),
    detectFormat: (source) => instance.detectFormat(source),
  };
