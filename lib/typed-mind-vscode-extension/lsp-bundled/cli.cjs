...
var scanOpaqueRun = (cursor, inGenericArgs = false, trackOwnAngles = false) => {
  const startIndex = cursor.index;
  const stack = [];
  const closerFor = { "(": ")", "[": "]", "{": "}" };
  let angleDepth = 0;
...
var parseAtom = (cursor, inGenericArgs = false) => {
  ...
  if (cursor.startsWith("(")) {
    ...
    if (cursor.startsWith(")")) {
      ...
      if (cursor.startsWith("=>")) {
        cursor.index = startIndex2;
        const text3 = scanOpaqueRun(cursor, inGenericArgs, true);
    ...
    cursor.index = startIndex2;
    const text2 = scanOpaqueRun(cursor, inGenericArgs, true);
