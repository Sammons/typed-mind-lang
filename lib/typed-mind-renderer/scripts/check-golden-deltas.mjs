...
// Every full graph-golden diagnostic/link class fires once per renderer
// class (interactive, enhanced, advanced) since all three read the same
// check() output through their own getGraphData implementation.
const RENDERER_CLASS_COUNT = 3;
...
// RFC-TM-14 U5a, leaf R6a (rfc-tm-14-diamond.md §S5, U-2): a Constants schema
// names the type of the value, which is a use, so schema references credit
// the orphan walk (the legacy port excluded schema by fidelity, not by rule —
// check-orphans.ts header). scenario-34's `taskDefinitions ! ... : TaskConstants`
// and `workerConstants ! ... : WorkerConstants` resolve both DTO orphans.
// lib/typed-mind-test-suite/src/constants-schema-corpus-deltas.test.ts proves
// removing only those schema slots restores exactly these findings.
const EXPECTED_R6A_SCHEMA_ORPHANS = new Map([
  ['scenario-34-cli-tool', ["Orphaned entity 'TaskConstants'", "Orphaned entity 'WorkerConstants'"]],
]);
...
// RFC-TM-14 §S2 (rfc-tm-14-diamond.md, U2 / R1a-lang): Function call
// entries are looked up through `QualifiedNameResolver.target` (the
// Constants arm's existing lookup, TM13 F+Q) instead of the raw call string,
// so a dotted entry (`Owner.method`, `Owner.constructor`) draws a `call`
// link to its resolved owner. Legacy drew nothing for a dotted Function
// call. An extra `call` link is this class when its source is a Function
// whose `calls` list carries an entry spelled `<target>.<member...>`. The
// per-document census (per renderer class; interactive + enhanced build
// call links, advanced stubs links: []) was measured on this branch:
//   imports-circular-module-a-b 2 (methodA -> ServiceB.methodB,
//   methodB -> ServiceA.methodA), scenario-21-aliased-import 1
//   (initializeApp -> DB.Connection.connect), scenario-31-mixed-syntax 1
//   (deleteTodo -> Database.delete), scenario-34-cli-tool 16,
//   scenario-35-video-game 42 = 62 per class.
const EXPECTED_TM14_U2_QUALIFIED_CALL_LINKS = new Map([
  ['imports-circular-module-a-b', 2],
  ['scenario-21-aliased-import', 1],
  ['scenario-31-mixed-syntax', 1],
  ['scenario-34-cli-tool', 16],
  ['scenario-35-video-game', 42],
]);
// The same lookup feeds GraphMetricsAnalyzer's dependency graph, so the two
// documents with many qualified calls (34, 35) gain dependency edges and
// their Coupling category (and the overall score derived from it) moves:
// scenario-34 Coupling 100 -> 99.84, Complexity 67.5 -> 65.5; scenario-35
// Coupling 100 -> 97.03, overall 79 -> 78. Downstream numeric consequences
// of TM14-U2-QUALIFIED-CALL-LINKS, not a class of their own.
const TM14_U2_METRICS_DOCS = new Set(['scenario-34-cli-tool', 'scenario-35-video-game']);

const isQualifiedCallLink = (link, live) => {
  if (link.type !== 'call') return false;
  const source = (live.entities ?? []).find((entity) => entity.name === link.source && entity.kind === 'Function');
  return source !== undefined && (source.calls ?? []).some((call) => call.startsWith(`${link.target}.`));
};
...
const classifyGraphLinks = (docName, rendererClass, baseline, live, report) => {
  ...
  for (const key of extra) {
    ...
    } else if (rendererClass !== 'advanced' && isQualifiedCallLink(link, live)) {
      report.tm14U2QualifiedCallLinks.set(docName, (report.tm14U2QualifiedCallLinks.get(docName) ?? 0) + 1);
    } else {
...
const classifyGraphErrors = (docName, baseline, live, report) => {
  ...
  for (const message of allMessages) {
    ...
    if (baselineCount > liveCount) {
      ...
      } else if (EXPECTED_R6A_SCHEMA_ORPHANS.get(docName)?.includes(message)) {
        report.r6aSchemaOrphanResolved += vanishedCount;
...
const main = () => {
  ...
  const report = {
    ...
    tm14U2QualifiedCallLinks: new Map(),
  };
  const unclassified = [];
  ...
  for (const relPath of baselineRel) {
    ...
    if (relPath.startsWith('graph/')) {
      ...
      } else if (TM14_U2_METRICS_DOCS.has(docName)) {
        // RFC-TM-14 U2: qualified call targets join the dependency graph
        // (see TM14_U2_METRICS_DOCS above); the link-level class is
        // counted in classifyGraphLinks and census-checked below.
  ...
  const expectedA11 = EXPECTED_A11_MESSAGES.length * RENDERER_CLASS_COUNT; // per renderer class
  ...
  const expectedDLeg8 = EXPECTED_D_LEG_8_VANISHED_MESSAGES.length * RENDERER_CLASS_COUNT; // per renderer class
  ...
  const expectedTm13Q1 = EXPECTED_TM13_Q1_VANISHED_MESSAGES.length * RENDERER_CLASS_COUNT; // per renderer class
  ...
  const expectedR6a = 6; // two exact messages across three renderer classes
  if (report.r6aSchemaOrphanResolved !== expectedR6a) {
    throw new GoldenDeltaError(
      `cause class R6A-SCHEMA-ORPHAN-RESOLVED: expected exact count ${expectedR6a}, got ${report.r6aSchemaOrphanResolved}`,
    );
  }

  const expectedA6Vanished = EXPECTED_A6_VANISHED_MESSAGES.length * RENDERER_CLASS_COUNT; // per renderer class
  const expectedA6New = EXPECTED_A6_NEW_MESSAGE_PREFIXES.length * 2 * RENDERER_CLASS_COUNT; // 2 messages/name x3 names, per renderer class
  const expectedA6DuplicateEntities = DUPLICATED_ENTITY_NAMES.length * RENDERER_CLASS_COUNT; // 1 extra copy/name x3 names, per renderer class
  ...
  const expectedA1 = EXPECTED_A1_COUNT * RENDERER_CLASS_COUNT; // per renderer class
  ...
  const expectedA10 = EXPECTED_A10_COUNT * RENDERER_CLASS_COUNT; // per renderer class
  ...
  const expectedA2 = EXPECTED_A2_COUNT * RENDERER_CLASS_COUNT; // 7 total occurrences, per renderer class (RFC-TM-8 §1 collateral, snippet text updated by tm10-inc2-grammar — see EXPECTED_A2_MESSAGES comment)
  ...
  for (const [docName, expectedPerClass] of EXPECTED_TM14_U2_QUALIFIED_CALL_LINKS) {
    const expected = expectedPerClass * 2; // interactive + enhanced
    const actual = report.tm14U2QualifiedCallLinks.get(docName) ?? 0;
    if (actual !== expected) {
      throw new GoldenDeltaError(`cause class TM14-U2-QUALIFIED-CALL-LINKS (${docName}): expected exact count ${expected}, got ${actual}`);
    }
  }
  for (const docName of report.tm14U2QualifiedCallLinks.keys()) {
    if (!EXPECTED_TM14_U2_QUALIFIED_CALL_LINKS.has(docName)) {
      throw new GoldenDeltaError(`cause class TM14-U2-QUALIFIED-CALL-LINKS: unexpected document ${docName}`);
    }
  }
  ...
  console.log(`  R6A-SCHEMA-ORPHAN-RESOLVED: ${report.r6aSchemaOrphanResolved} (expected ${expectedR6a})`);
  ...
  console.log(
    `  TM14-U2-QUALIFIED-CALL-LINKS: ${[...report.tm14U2QualifiedCallLinks].map(([doc, count]) => `${doc}=${count}`).join(', ')} (expected ${[...EXPECTED_TM14_U2_QUALIFIED_CALL_LINKS].map(([doc, count]) => `${doc}=${count * 2}`).join(', ')})`,
  );
};
