...
// unrecognized-form blocks) surface as only 8 UNIQUE messages: tree-sitter's
// error-node grouping merges two adjacent unparsable regions into one ERROR
// node when nothing parseable separates them, and PluginAPI (470-474) /
// Logger (476-480) are adjacent with only a blank line (475, not flagged)
// between them — so their `unrecognized-form` blocks collapse into a single
// `syntax/error` diagnostic spanning both, leaving FileUtils and
// ProcessUtils each with their own. 8 unique messages x 3 renderer classes
// (interactive/enhanced/advanced all read the same check() diagnostics) = 24.
const EXPECTED_A2_MESSAGES = [
  ...
  'unparsable text: `FileUtils < "File utilities interface"`',
  'unparsable text: `ProcessUtils < "Process utilities interface"`',
];
const EXPECTED_A2_DOC = 'scenario-34-cli-tool';
const EXPECTED_A2_COUNT = 8; // unique messages (see occurrence counts inline above)
...
const main = () => {
  ...
  const expectedA2 = EXPECTED_A2_COUNT * 3; // 8 unique messages x3 renderer classes
