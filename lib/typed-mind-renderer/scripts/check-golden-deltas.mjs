...
const EXPECTED_A1_MESSAGE = 'illegal continuation: imports list (`<- [...]`) cannot attach to a Class entity';
...
// is frozen at exactly 18 records.
const EXPECTED_A10_MESSAGE = 'illegal continuation: affects list (`~ [...]`) cannot attach to a UIComponent entity';
...
//
// RFC-TM-8 §1 collateral (rfc-tm-8-diamond.md, X-TYPE-1): this Quantum's
// type-expression sub-grammar adds new token shapes to the grammar's global
// token table, which shifts what GLR error recovery can match at every
// recovery point (the same mechanism documented on q1-shortform.txt's/
// q2-longform.txt's collateral ERROR-recovery-shape notes). The recovery
// boundary now ALSO merges FileUtils (482-486) with ProcessUtils (488-493)
// across their shared blank line (487) — the same "only a blank line
// between them" shape PluginAPI/Logger already merge across. ProcessUtils's
// unrecognized-form block no longer produces its OWN diagnostic; its text
// is absorbed into FileUtils's existing `syntax/error` span, so the total
// OCCURRENCE count (not the unique-message count, which stays at 4 — the
// FileUtils message string is unchanged) drops from 8 to 7: `[]`x3 +
// `, TaskContext`x2 + `PluginAPI < ...`x1 (Logger folded in) + `FileUtils <
// ...`x1 (ProcessUtils now folded in too, replacing what used to be its own
// 8th occurrence). The document-level verdict is unchanged: scenario-34 is
// still invalid, still flagged. 7 total occurrences x 3 renderer classes
// (interactive/enhanced/advanced all read the same check() diagnostics) = 21.
const EXPECTED_A2_MESSAGES = [
  'unparsable text: `[]`', // x3 occurrences (lines 221, 227, 239)
  'unparsable text: `, TaskContext`', // x2 occurrences (lines 250, 290)
  'unparsable text: `PluginAPI < "Plugin API interface"`', // Logger's block folds in (tree-sitter grouping)
  'unparsable text: `FileUtils < "File utilities interface"`', // ProcessUtils's block now folds in too (RFC-TM-8 collateral, see above)
];
const EXPECTED_A2_DOC = 'scenario-34-cli-tool';
const EXPECTED_A2_COUNT = 7; // total occurrences across the 4 unique messages above (see inline counts)
...
const main = () => {
  ...
  const expectedA2 = EXPECTED_A2_COUNT * 3; // 7 total occurrences x3 renderer classes (RFC-TM-8 §1 collateral, see EXPECTED_A2_MESSAGES comment)
