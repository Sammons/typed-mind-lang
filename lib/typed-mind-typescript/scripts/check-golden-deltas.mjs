...
// version/exports. The Program's own NAME is read off the live block's own
// header rather than the baseline line, because this delta composes with
// PROGRAM-NAME-COLLISION-FIX on the same fixtures (RFC-TM-9 X-CONV-4's
// `<Base>App` -> `<Base>__App` rename already changed the name independent
// of RC-C) — this matcher only asserts entry/version/exports carry over
// unchanged, which is what RC-C's fix actually promises; the name is
// whatever the live emission's own header says. Every other block-body line
// is checked verbatim against the values captured off the baseline's own
// two lines, so this can only match the exact shape RC-C's fix produces —
// any other divergence still falls through to unclassified.
...
const matchProgramExportsLongformPromotion = (baselineLines, baselineIndex, liveLines, liveIndex) => {
  ...
  const [, entry, version] = declarationMatch;
