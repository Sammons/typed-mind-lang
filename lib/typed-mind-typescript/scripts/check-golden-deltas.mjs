...
const CAUSE_CLASSES = {
  ...
  'PROGRAM-NAME-COLLISION-FIX': { expected: 2 }, // RFC-TM-9 X-CONV-4: <Base>App -> <Base>__App, one per golden with a Program (preferClassFile-true, preferClassFile-false)
};
...
const classifyFileDelta = (relPath, baselineContent, liveContent, censusCounts, unclassified) => {
  ...
  while (baselineIndex < baseline.lines.length || liveIndex < live.lines.length) {
    ...
    if (
      baselineIndex < baseline.lines.length &&
      liveIndex < live.lines.length &&
      isProgramNameCollisionFix(baselineLine, liveLine)
    ) {
