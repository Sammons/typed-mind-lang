...
// which default to sibling/self locations when omitted (see ARG_DEFAULTS
// below) — pass them explicitly when running from a different checkout.
//
...
const { values: args } = parseArgs({
  options: {
    ...
    'claude-home': { type: 'string', default: resolve(REPO_ROOT, '..', 'claude-home') },
...
const runNode = (scriptPath, cliArgs, cwd) => {
  const result = spawnSync('/usr/bin/node', [scriptPath, ...cliArgs], { cwd, encoding: 'utf8' });
  return result;
};
...
const countSuppressions = (tmdPath) => {
  ...
  const counts = {};
  const suppressionRe = /suppress(?:ed)?:\s*(['"])([a-z-]+)\1/gi;
...
const runExtraction = (target, command, cliArgs, cwd, outputName, extraNotes = []) => {
  ...
  if (!exportSuccess) {
    notes.push(`extractor exit ${result.status}: ${(result.stderr || result.stdout || '').split('\n').slice(0, 3).join(' | ')}`);
  }
  return {
    target,
    command,
...
const installWebhookstorageDeps = (cloneDir) => {
  ...
  spawnSync('/usr/bin/npm', ['install', '--ignore-scripts', '--no-audit', '--no-fund', '--legacy-peer-deps'], {
    cwd: cloneDir,
    stdio: 'inherit',
  });
  for (const pkgName of packageDirs) {
