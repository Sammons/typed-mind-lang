...
// automated — see installWebhookstorageDeps() below.
//
// issue #98 — the workaround used to rewrite the CALLER-SUPPLIED clone's
// own tracked package.json files in place and run `npm install` directly
// inside it, leaving mutated package.json + untracked package-lock.json/
// node_modules behind on every run. A second run against the
// already-mutated tree then failed outright (npm choked on its own
// prior-run leftovers) and, when it partially succeeded, produced
// diagnostic-count deltas that looked like real regressions but were
// purely an artifact of the corrupted install. Fix: the workaround now
// runs against a SCRATCH COPY of the clone (`--out-dir`/webhookstorage-scratch),
// made fresh on every invocation via `cp -a` from the caller-supplied
// clone. The caller's own long-lived checkout is never written to, so the
// same clone can be reused indefinitely across ladder runs, and re-running
// the ladder twice back-to-back against the same clone produces identical
// reports.
...
// Makes a fresh scratch copy of the caller-supplied clone under `--out-dir`
// so installWebhookstorageDeps() below has a tree it can freely mutate.
// Any leftover scratch copy from a prior run in the same --out-dir is
// removed first, so every invocation starts from a byte-for-byte copy of
// the CURRENT state of the caller's clone — never a stale, previously
// installed-into copy.
const makeWebhookstorageScratchCopy = (cloneDir, outDir) => {
  const scratchDir = join(outDir, 'webhookstorage-scratch');
  rmSync(scratchDir, { recursive: true, force: true });
  mkdirSync(scratchDir, { recursive: true });
  cpSync(cloneDir, scratchDir, {
    recursive: true,
    // dereference: true is required for scratch-copy independence. cpSync
    // defaults to dereference:false, which copies a symlink AS a symlink
    // still pointing at the ORIGINAL clone's absolute path rather than
    // dereferencing its target into the scratch tree. A stray symlink
    // anywhere in the source clone outside node_modules/.git (unlikely
    // today, but not guaranteed never to appear) would otherwise leave the
    // scratch copy silently reading — or, once the source clone moves or
    // is deleted, unable to read — content outside the copy it is supposed
    // to be fully independent of. Found in PR #106 review.
    dereference: true,
    filter: (src) => {
      // Skip node_modules and .git — neither is needed for the extractor's
      // static analysis, and both are large enough that copying them would
      // meaningfully slow the ladder down for no benefit. package-lock.json
      // is skipped too: this workaround always regenerates one from
      // scratch on the copy, so any stale/absent lockfile in the source
      // clone is irrelevant.
      const base = basename(src);
      return base !== 'node_modules' && base !== '.git' && base !== 'package-lock.json';
    },
  });
  return scratchDir;
};
...
// issue #98 — install the workaround's dependency mutations into a fresh
// scratch copy, never into WEBHOOKSTORAGE_CLONE itself, so the
// caller-supplied clone stays untouched and re-runnable indefinitely.
const WEBHOOKSTORAGE_SCRATCH = makeWebhookstorageScratchCopy(WEBHOOKSTORAGE_CLONE, OUT_DIR);
installWebhookstorageDeps(WEBHOOKSTORAGE_SCRATCH);
...
for (const ep of webhookstorageEntrypoints) {
  const args = [
    '--project',
    join(WEBHOOKSTORAGE_SCRATCH, ep.project),
    '--entrypoint',
    join(WEBHOOKSTORAGE_SCRATCH, ep.entry),
    '--recognize',
    'sst-handler',
  ];
  results.push(
