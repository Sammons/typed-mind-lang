...
describe('scenario-31-mixed-syntax', () => {
  ...
  it('should fail validation due to orphaned entities', () => {
    ...
    const classNotExportedError = result.errors.find((err) => err.message === "Class 'Database' is not exported by any file");
    ...
    assert.equal(
      ...
      "Add 'Database' to the exports of a file entity or convert to ClassFile with #: operator",
    );
    assert.equal(classNotExportedError?.position.line, 74);

    // Check for UIComponent not contained error
    const uiComponentError = result.errors.find(
      (err) => err.message === "UIComponent 'UserProfile' is not contained by any other UIComponent",
    );
    assert.notEqual(uiComponentError, undefined);
    assert.equal(uiComponentError?.severity, 'error');
    assert.equal(
      uiComponentError?.suggestion,
      "Either add 'UserProfile' to another UIComponent's contains list, or mark it as a root component with &!",
    );
