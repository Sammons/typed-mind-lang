...
describe('Scenario 63: File isolation patterns', () => {
  ...
  it('should handle selective exports from public API', () => {
    ...
    const publicAPI = Array.from(parseResult.entities.values()).find((e) => e.name === 'PublicAPI' && e.type === 'File');
  ...
  it('should enforce module boundaries', () => {
    ...
    const authModule = Array.from(parseResult.entities.values()).find((e) => e.name === 'AuthModule' && e.type === 'File');
  ...
  it('should handle layered architecture', () => {
    ...
    const presentation = Array.from(parseResult.entities.values()).find((e) => e.name === 'PresentationLayer' && e.type === 'File');
    ...
    const business = Array.from(parseResult.entities.values()).find((e) => e.name === 'BusinessLayer' && e.type === 'File');
    ...
    const data = Array.from(parseResult.entities.values()).find((e) => e.name === 'DataLayer' && e.type === 'File');
  ...
  it('should detect orphaned functions', () => {
    ...
    const implDetail = Array.from(parseResult.entities.values()).find((e) => e.name === 'ImplementationDetail' && e.type === 'File');
  ...
  it('should handle re-export patterns', () => {
    ...
    const coreExports = Array.from(parseResult.entities.values()).find((e) => e.name === 'CoreExports' && e.type === 'File');
  ...
  it('should handle files with no exports', () => {
    ...
    // Side effects file
    const noExportFile = Array.from(parseResult.entities.values()).find((e) => e.name === 'NoExportFile' && e.type === 'File');
    ...
    const importOnlyFile = Array.from(parseResult.entities.values()).find((e) => e.name === 'ImportOnlyFile' && e.type === 'File');
  ...
  it('should handle test file isolation', () => {
    ...
    const testFile = Array.from(parseResult.entities.values()).find((e) => e.name === 'TestFile' && e.type === 'File');
    ...
    const testSuite = Array.from(parseResult.entities.values()).find((e) => e.name === 'testSuite' && e.type === 'Function');
  ...
  it('should handle environment-specific files', () => {
    ...
    // Dev-specific file
    const devFile = Array.from(parseResult.entities.values()).find((e) => e.name === 'DevFile' && e.type === 'File');
    assert.ok((devFile?.path).includes('dev/'));

    // Prod-specific file
    const prodFile = Array.from(parseResult.entities.values()).find((e) => e.name === 'ProdFile' && e.type === 'File');
    assert.ok((prodFile?.path).includes('prod/'));

    // Different entry points
    const devEntry = Array.from(parseResult.entities.values()).find((e) => e.name === 'DevEntry' && e.type === 'File');
    const prodEntry = Array.from(parseResult.entities.values()).find((e) => e.name === 'ProdEntry' && e.type === 'File');
  ...
  it('should validate file isolation integrity', () => {
    const parseResult = parser.parse(content);
    const _validationResult = validator.validate(parseResult.entities, parseResult);

    // Check that internal services are not orphaned
    const internalService = Array.from(parseResult.entities.values()).find((e) => e.name === 'InternalService' && e.type === 'ClassFile');

    // InternalService is imported by PublicAPI
    assert.notEqual(internalService, undefined);

    // Private functions should be traceable
    const privateHelper = Array.from(parseResult.entities.values()).find((e) => e.name === 'PrivateHelper' && e.type === 'File');
  ...
  it('should validate all layer dependencies', () => {
    ...
    const handleRequest = Array.from(parseResult.entities.values()).find((e) => e.name === 'handleRequest' && e.type === 'Function');
    assert.ok((handleRequest?.calls).includes('processBusinessLogic'));

    const processBusinessLogic = Array.from(parseResult.entities.values()).find(
      (e) => e.name === 'processBusinessLogic' && e.type === 'Function',
    );
