...
describe('Scenario 48: Constants Edge Cases', () => {
  it('should validate Constants with and without schema', () => {
    ...
    const orphanedSchemaError = result.errors.find((e) => e.message.includes('ConfigSchema') && e.message.includes('Orphaned entity'));
    ...
    const _schemaErrors = result.errors.filter((e) => e.message.includes('NonExistentSchema'));
    // We may or may not get a specific "NonExistentSchema not found" error

    // Constants without schema should be valid (no error)
    const noSchemaError = result.errors.find((e) => e.message.includes('NoSchemaConfig'));
    assert.equal(noSchemaError, undefined);

    // Valid constants with schema should be valid (no error)
    const validConfigError = result.errors.find((e) => e.message.includes('ValidConfig') && !e.message.includes('NonExistentSchema'));
    ...
    const entityNotFoundErrors = result.errors.filter(
      (e) =>
