...
describe('Scenario 54: Entity Name Boundaries', () => {
  it('should validate entity naming rules and boundaries', () => {
    ...
    const numberStartError = result.errors.find((e) => e.message.includes('123Name') && e.message.includes('Orphaned'));
    ...
    assert.equal(
      errorMessages.some((msg) => msg.includes('123Name')),
      true,
    );
    ...
    const validNameError = result.errors.find((e) => e.message.includes('ValidName') && !e.message.includes('Orphaned'));
    assert.equal(validNameError, undefined);

    // Long names should be valid (no arbitrary length limit)
    const longNameError = result.errors.find((e) => e.message.includes('VeryLongEntityName') && e.message.includes('too long'));
