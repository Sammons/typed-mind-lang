...
describe('Scenario 50: Circular Function Calls', () => {
  it('should detect circular function call dependencies', () => {
    ...
    const orphanedFile = result.errors.find((e) => e.message.includes("Orphaned file 'SecondaryFile'"));
    assert.notEqual(orphanedFile, undefined);

    // Deep chain without circular should NOT have errors
    const deepChainError = result.errors.find(
      (e) => e.message.includes('chainStart') || e.message.includes('chainMiddle') || e.message.includes('chainEnd'),
    );
