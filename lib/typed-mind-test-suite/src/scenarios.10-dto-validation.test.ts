...
describe('scenario-10-dto-validation', () => {
  ...
  it('should validate DTO structure and detect validation errors', () => {
    ...
    const orphanedUpdateUser = result.errors.find((err) => err.message === "Orphaned entity 'updateUser'");
    assert.notEqual(orphanedUpdateUser, undefined);
    assert.equal(orphanedUpdateUser?.position.line, 10);

    // Check for validation errors
    const cannotUseInputError = result.errors.find((err) => err.message === "Cannot use 'input' to reference File 'UserFile'");
    ...
    const nonExistentDTOError = result.errors.find((err) => err.message === "Function input DTO 'NonExistentDTO' not found");
    assert.notEqual(nonExistentDTOError, undefined);
    assert.equal(nonExistentDTOError?.position.line, 6);

    const userFileNotDTOError = result.errors.find((err) => err.message === "Function input 'UserFile' is not a DTO (it's a File)");
    assert.notEqual(userFileNotDTOError, undefined);
    assert.equal(userFileNotDTOError?.position.line, 10);

    const notADTOError = result.errors.find((err) => err.message === "Function output DTO 'NotADTO' not found");
