...
describe('scenario-13-asset-validation', () => {
  ...
  it('should validate asset entities and their relationships', () => {
    ...
    const orphanedAssetsFileError = result.errors.find(
      (err) =>
    ...
    const orphanedUnexportedAssetError = result.errors.find(
      (err) => err.message === "Orphaned entity 'UnexportedAsset'" && err.position.line === 15 && err.position.column === 1,
    );
