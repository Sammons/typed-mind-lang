...
describe('Scenario 65: Bidirectional consumedBy for RunParameters', () => {
  ...
  it('should automatically populate RunParameter.consumedBy when Function consumes it', () => {
    ...
    const databaseUrl = Array.from(parseResult.entities.values()).find(
      (e) => e.name === 'DATABASE_URL' && e.type === 'RunParameter',
    ) as any;
  ...
  it('should handle single function consuming RunParameter', () => {
    ...
    const apiKey = Array.from(parseResult.entities.values()).find((e) => e.name === 'API_KEY' && e.type === 'RunParameter') as any;
  ...
  it('should handle RunParameter with no consuming functions', () => {
    ...
    const unusedParam = Array.from(parseResult.entities.values()).find(
      (e) => e.name === 'UNUSED_PARAM' && e.type === 'RunParameter',
    ) as any;
  ...
  it('should maintain consistency between Function.consumes and RunParameter.consumedBy', () => {
    ...
    const connectDB = Array.from(parseResult.entities.values()).find((e) => e.name === 'connectDB' && e.type === 'Function') as any;
    const databaseUrl = Array.from(parseResult.entities.values()).find(
      (e) => e.name === 'DATABASE_URL' && e.type === 'RunParameter',
    ) as any;
  ...
  it('should handle different RunParameter types', () => {
    ...
    const port = Array.from(parseResult.entities.values()).find((e) => e.name === 'PORT' && e.type === 'RunParameter') as any;
    const dbTimeout = Array.from(parseResult.entities.values()).find((e) => e.name === 'DB_TIMEOUT' && e.type === 'RunParameter') as any;
    const maxRetries = Array.from(parseResult.entities.values()).find((e) => e.name === 'MAX_RETRIES' && e.type === 'RunParameter') as any;
  ...
  it('should validate without errors when bidirectional relationships are correct', () => {
    ...
    const consumedByErrors = validationResult.errors.filter((e) => e.message.includes('consumedBy'));
