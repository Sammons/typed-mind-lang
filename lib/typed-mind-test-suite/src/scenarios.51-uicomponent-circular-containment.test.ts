...
describe('Scenario 51: UIComponent Circular Containment', () => {
  it('should detect circular UIComponent containment', () => {
    ...
    const selfContainError = result.errors.find(
      (e) => e.message.includes('SelfContainer') && (e.message.includes('itself') || e.message.includes('circular')),
    );
    ...
    const directCircularError = result.errors.find(
      (e) => (e.message.includes('CircularA') || e.message.includes('CircularB')) && e.message.includes('circular'),
    );
    assert.notEqual(directCircularError, undefined); // Circular containment should be detected

    // Three-way circular containment
    const threeWayError = result.errors.find(
      (e) =>
        (e.message.includes('ThreeWayA') || e.message.includes('ThreeWayB') || e.message.includes('ThreeWayC')) &&
    ...
    const complexCircularError = result.errors.find(
      (e) => (e.message.includes('ComplexCircle1') || e.message.includes('ComplexCircle2')) && e.message.includes('circular'),
    );
    ...
    const validRootError = result.errors.find(
      (e) => e.message.includes('ValidRoot') && !e.message.includes('InvalidRoot') && !e.message.includes('Orphaned'), // Ignore orphaned entity errors
    );
