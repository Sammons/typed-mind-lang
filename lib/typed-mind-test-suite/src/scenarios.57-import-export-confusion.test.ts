...
describe('Scenario 57: Import and export confusion', () => {
  ...
  it('should detect import/export mistakes', async () => {
    ...
    assert.equal(
      errors.some((e) => e.includes('Circular import detected') && e.includes('UserService')),
      true,
    );

    // Mistake 2: Import non-existent
    assert.equal(
      errors.some((e) => e.includes("Import 'NonExistentModule' not found")),
      true,
    );

    // Mistake 3: Export undefined
    assert.equal(
      errors.some((e) => e.includes("Export 'deleteConfig' is not defined anywhere")),
      true,
    );
    ...
    assert.equal(
      errors.some((e) => e.includes("Orphaned entity 'Logo'")),
      true,
    );
    assert.equal(
      errors.some((e) => e.includes("Orphaned entity 'Button'")),
      true,
    );
    ...
    assert.equal(
      errors.some((e) => e.includes('Circular import detected: A -> B -> C -> A')),
      true,
    );
  });
