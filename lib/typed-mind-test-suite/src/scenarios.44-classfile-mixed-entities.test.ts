...
describe('scenario-44-classfile-mixed-entities', () => {
  ...
  it('should handle mixed ClassFile and regular entities correctly', () => {
    ...
    const baseControllerOrphanedError = result.errors.find(
      (err) => err.message.includes('BaseController') && err.message.includes('Orphaned entity'),
    );
    ...
    const baseControllerNotExportedError = result.errors.find(
      (err) => err.message.includes('BaseController') && err.message.includes('is not exported by any file'),
    );
    ...
    const dataProcessorError = result.errors.find(
      (err) => err.message.includes('DataProcessor') && err.message.includes('is not exported by any file'),
    );
