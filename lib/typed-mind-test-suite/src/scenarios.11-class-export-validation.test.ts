...
describe('scenario-11-class-export-validation', () => {
  ...
  it('should validate class export behavior and detect export violations', () => {
    ...
    const orphanedUnexportedClass = result.errors.find((err) => err.message === "Orphaned entity 'UnexportedClass'");
    assert.notEqual(orphanedUnexportedClass, undefined);
    assert.equal(orphanedUnexportedClass?.position.line, 7);

    const orphanedUnexportedFunction = result.errors.find((err) => err.message === "Orphaned entity 'unexportedFunction'");
    assert.notEqual(orphanedUnexportedFunction, undefined);
    assert.equal(orphanedUnexportedFunction?.position.line, 10);

    const orphanedExportedClass = result.errors.find((err) => err.message === "Orphaned entity 'ExportedClass'");
    assert.notEqual(orphanedExportedClass, undefined);
    assert.equal(orphanedExportedClass?.position.line, 13);

    const orphanedExportedFunction = result.errors.find((err) => err.message === "Orphaned entity 'exportedFunction'");
    ...
    const classNotExportedError = result.errors.find((err) => err.message === "Class 'UnexportedClass' is not exported by any file");
    ...
    assert.equal(
      ...
      "Add 'UnexportedClass' to the exports of a file entity or convert to ClassFile with #: operator",
    );

    const functionNotExportedError = result.errors.find(
      (err) => err.message === "Function 'unexportedFunction' is not exported by any file and is not a class method",
    );
    ...
    assert.equal(
      ...
      "Either add 'unexportedFunction' to the exports of a file entity or define it as a method of a class",
    );
  });
});
