...
describe('Scenario 64: Bidirectional affectedBy for UIComponents', () => {
  ...
  it('should automatically populate UIComponent.affectedBy when Function affects it', () => {
    ...
    const userList = Array.from(parseResult.entities.values()).find((e) => e.name === 'UserList' && e.type === 'UIComponent') as any;
  ...
  it('should handle single function affecting UIComponent', () => {
    ...
    const button = Array.from(parseResult.entities.values()).find((e) => e.name === 'Button' && e.type === 'UIComponent') as any;
  ...
  it('should maintain consistency between Function.affects and UIComponent.affectedBy', () => {
    ...
    const updateUserList = Array.from(parseResult.entities.values()).find(
      (e) => e.name === 'updateUserList' && e.type === 'Function',
    ) as any;
    const userList = Array.from(parseResult.entities.values()).find((e) => e.name === 'UserList' && e.type === 'UIComponent') as any;
  ...
  it('should validate without errors when bidirectional relationships are correct', () => {
    ...
    const affectedByErrors = validationResult.errors.filter((e) => e.message.includes('affectedBy'));
  ...
  it('should handle root UIComponent with affectedBy', () => {
    ...
    const dashboard = Array.from(parseResult.entities.values()).find((e) => e.name === 'Dashboard' && e.type === 'UIComponent') as any;
